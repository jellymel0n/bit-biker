...
class BGLayer {
    ...
    updatePosition(elapsedTimeMS = 1000) {
        const currentLeft = this.css("background-position-x");
        const currentTop = this.css("background-position-y");
        const newLeft =
          parseFloat(currentLeft.substr(0, currentLeft.length - 2)) +
          (this.velocity[0] / 1000) * elapsedTimeMS;
        const newTop =
          parseFloat(currentTop.substr(0, currentTop.length - 2)) +
          (this.velocity[1] / 1000) * elapsedTimeMS;
