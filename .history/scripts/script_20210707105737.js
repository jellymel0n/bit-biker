...
const game = {
    ...
    animationFrameLoop: (ts) => {
        ...
        if (ts - game.loopDuration > game.lastKeyFrame) {
            ...
            for (const gplayer of game.players) {
                gplayer.updatePosition(elapsedMSeconds);
            }
        } 
        window.requestAnimationFrame(game.animationFrameLoop);
    } 
...
class Player {
    ...
    updatePosition(elapsedTimeMS = 1000) {
        if(game.pressedKeys.indexOf(32) >= 0 && !this.isJumping) {
          if(this.currentJumpVelocity < this.maxJumpVelocity) {
            this.currentJumpVelocity += 10;
            this.$dom.css('transform', 'scaleY(0.7)');
          }
        } else if (this.currentJumpVelocity > 0) {
          this.isJumping = true;
          this.$dom.css('transform', 'scaleY(1.0)');
          this.velocity[1] = this.currentJumpVelocity * -1;
